...
import { Pulse, ShieldWarning } from "@phosphor-icons/react";
...
export default function Dashboard() {
  ...
  const poll = useCallback(async () => {
    try {
      ...
      // Auto-restore session token from wallet payload on every poll
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      <header className="px-4 py-3 flex flex-wrap items-center justify-between gap-4 border-b border-neutral-100">
        <div className="flex items-center gap-3">
          <ShieldWarning size={20} weight="regular" className="text-indigo-300" />
          <div>
        ...
        <div className="flex items-center gap-4 text-sm text-neutral-500">
          <span>{tokens.length} watched</span>
      ...
      {backendOk === false && (
        <div className="mx-4 mb-2 px-3 py-2 rounded-[3px] bg-red-50 text-sm text-red-600">
      ...
      <ErrorBoundary>
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden gap-2 p-4">
        <aside className="w-full md:w-64 md:shrink-0 panel flex flex-col">
          <div className="px-2 pt-2 pb-1">
            <p className="label-col px-1 mb-2">Watchlist</p>
            <div className="overflow-y-auto max-h-48 md:max-h-[calc(100vh-280px)]">
              <WatchList
                tokens={tokens}
        ...
        <main className="flex-1 panel overflow-y-auto p-3 flex flex-col gap-3 min-w-0">
          {!selected ? (
            <EmptyState />
          ) : (
            <>
                ...
                {!selected.exited && (
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={simulating}
                      className="text-xs font-medium px-2.5 py-1 rounded-lg status-danger disabled:opacity-50"
                    >
        ...
        <aside className="hidden md:flex md:w-72 md:shrink-0 panel flex-col">
          <p className="label-col px-3 pt-2 pb-2">All events</p>
...
function LoadingSkeleton() {
  return (
    <div className="flex-1 flex gap-2 p-2 animate-pulse">
      <div className="w-64 shrink-0 panel h-64 bg-neutral-100 rounded-2xl" />
      <div className="flex-1 panel h-64 bg-neutral-100 rounded-2xl" />
      <div className="w-72 shrink-0 panel h-64 bg-neutral-100 rounded-2xl" />
    </div>
  );
}
